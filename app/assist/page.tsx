    ...
    async function callLLM(question: string) {
        ...
        try {
            ...
            const response = await fetch('/api/getLLMResponse', {
                method: 'POST',
                body: JSON.stringify({query: question, chatHistory: currentChatArrayRef.current, detailMode:detailMode,modelType: modelType}),
                headers: {
                    'Content-Type': 'application/json'
                }
            });
            ...
            while (true) {
                ...
                if (first) {
                    setCurrentChat((state) => [
                        ...state,
                        { message: "", sender: "AI", messageType: "ANSWER", messageId: '', rating:-1, reviewComments: "",modelType: modelType , commentAddedAt: null } // Temporary ID
                    ]);
                    first = false;
                }
                ...
                {status === "authenticated" &&
                    <>
                        <div className={`flex-1 p-1 ${currentChat.length !== 1 ? "max-w-5xl" : "max-w-8xl"} flex flex-col items-center`}>
                            ...
                            {currentChat.length > 1 ?
                            <div className='justify-center items-center text-gray-400 text-xs m-2'>AdvisorGPT can make mistakes. Please check important info.</div> : ''}
