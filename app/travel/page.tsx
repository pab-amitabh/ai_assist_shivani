...
export default function CommandActivation() {
    ...
    const [modelType, setModelType] = useState("travel")
    const [eligibilityWelcomeText, setEligibilityWelcomeText] = useState("Travel Eligibility GPT");
        ...
        if (llmDone) {
            ...
            const updateChat = async (newMessage: string, messageType: string, sender: string, questionId: string | null, isResolved: boolean) => {
                const response = await fetch("/api/updateChat", {
                    ...
                    body: JSON.stringify({ chatId: currChatId, message: newMessage, messageType: messageType, sender: sender, questionId: questionId, isResolved: isResolved,modelType: modelType })
                })
    ...
    async function callLLM(question: string) {
        ...
        try {
            const response = await fetch('/api/getLLMResponse', {
                ...
                body: JSON.stringify({
                    query: question,
                    chatHistory: currentChatArrayRef.current,
            ...
            while (true) {
                ...
                if (first) {
                    setCurrentChat((state) => [
                        ...state,
                        { message: "", sender: "AI", messageType: "ANSWER", messageId: '', rating: -1, reviewComments: "", commentAddedAt: null, modelType: modelType } // Temporary ID
                    ]);
                    first = false;
                }
    ...
    const LoadingDots = () => {
        return (
            <div className="flex w-full max-w-md ml-4 p-4 items-center bg-white rounded-lg">
                ...
                <>
                    <div className={`flex-1 p-1 ${currentChat.length !== 1 ? "max-w-5xl" : "max-w-8xl"} flex flex-col items-center`}>
                        ...
                        {currentChat.length === 1 ?
                            <>
                                <div className="mt-32 w-full bg-white rounded-lg flex flex-col md:flex-row text-center">
                                    <div className="md:ml-6 flex-1">
                                        <h2 className="text-3xl font-extrabold text-gray-900">
                                            Welcome to {eligibilityWelcomeText}
                                        </h2>
                        ...
                        {/* Input box shows only when eligibility mode is OFF or Travel Eligibility button is clicked */}
                            ...
                            <form onSubmit={getResponse} className={`sticky bottom-0 ${currentChat.length === 1 ? "mt-20" : "mt-2 mb-2"} w-full max-w-4xl flex items-center bg-white border border-gray-300 rounded-lg shadow-md p-3`}>
                                <input
                                    ...
                                    placeholder={loading ? "Analyzing" : "Enter client info including age & condition..."}
                        ...
                        {currentChat.length > 1 ?
                            <div className='justify-center items-center text-gray-400 text-xs m-2'>Travel Eligibility GPT can make mistakes. Please check important info.</div> : ''}
                    </div>
